...
class PanelPrincipalVisualizacionRadiologo():
    ...
    def crear_textbox(self,parent, font, campo_col, fila, columna, ancho=1, alto_widget=65, ancho_widget=30, valor_db=None):
        ...
        elif campo_col == "comentarios del radiologo"  and valor_real.strip() != "":
        ...
        for fila_paciente, paciente in enumerate(pacientes_filtrados):
            ...
            for i, campo2 in enumerate(campos2):
                ...
                if campo2["tipo"] == "entry":
                    ...
                    if campo2["columna"] == "comentarios del radiologo":
                        pac_id = paciente['identificacion_paciente']
                        if pac_id not in self.textbox_por_paciente:
                            self.textbox_por_paciente[pac_id] = {}

                        self.textbox_por_paciente[pac_id]['comentarios_del_radiologo'] = textbox
    ...
    def comentar_paciente(self, paciente):
        ...
        # DEBUG: paciente recibido
        print("DEBUG paciente recibido:", paciente)

        # Obtener comentario del radiologo desde el textbox
        pac_id = paciente['identificacion_paciente']
        textbox = self.textbox_por_paciente[pac_id]['comentarios_del_radiologo']
        coment_radiologo = textbox.get("1.0", "end-1c").strip()

        # Solo usar los datos del diccionario paciente para no modificar otros campos
        identificacion = paciente["identificacion_paciente"]

        # Obtener id_estado de Comentado
        ...
        self.db.cursor.execute(sql, (id_estado, coment_radiologo, UsuarioActual.id_usuario, identificacion))

        # Insertar en tabla modificados usando solo los valores existentes
        valores1 = (
    ...
    def aviso_radiologo(self):
        ...
        if not self.db.conexion.is_connected():
            try:
                ...
                self.after_id_aviso = self.ventana.after(600000, self.aviso_radiologo)
                return
    ...
    def salir(self):
        ...
        # Cancelar aviso_radiologo si está activo
        try:
