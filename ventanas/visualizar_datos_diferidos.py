...
from abrirventanas.abrir import abrir_ventana_visualizar_datos_ppal_realizados, abrir_ventana_visualizar_datos_ppal
...
class PanelPrincipalVisualizacionDiferidos():
    ...
    def __init__(self, frame_sup, frame, parent_window=None):
        ...
        if not PanelPrincipalVisualizacionDiferidos.conexion_realizada:
            try:
                PanelPrincipalVisualizacionDiferidos.db = Conexion_DB()
                PanelPrincipalVisualizacionDiferidos.db.conectar()
                abrir_ventana_conn_exito()
                PanelPrincipalVisualizacionDiferidos.conexion_realizada = True
            except Exception:
        ...
        self.db = PanelPrincipalVisualizacionDiferidos.db
    ...
    def cargar_pacientes(self):
        ...
        consulta_pacientes = "SELECT * FROM registrospacientesdiferidos ORDER BY hora_citacion"
    ...
    def visual_principal_titulo(self):
        ...
        campos = [
            ...
            {"label": "Visualización De Pacientes Diferidos", "valor": "", "ancho": 400, "tipo": "label"},
        ...
        for i, campo in enumerate(campos1):
            ...
            if campo['tipo'] == 'boton':
                ...
                self.crear_boton(visual_datos_ppal1, self.fonts['boton'], campo['label'], campo['color'], 0, i*2+1, campo['alto'], campo['ancho'], command=campo['command'], image=campo.get('image', None))
    ...
    def obtener_pacientes_filtrados(self):
        ...
        id_diferido = self.obtener_id_estado("Diferido")
        pacientes_por_sede = [
            paciente for paciente in self.datos_db
            if paciente.get("sede") == id_sede
            and paciente.get("estado", "") in ((id_diferido, ))
        ]
