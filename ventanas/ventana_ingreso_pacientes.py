...
class IngresarPacientes():
    ...
    def llenar_combobox_rango_edad(self):
        ...
        print("Datos obtenidos de DB:", informacion)
        
        # Extraer solo los nombres de rango_edad
        opciones = [fila["rango"] for fila in informacion]
        print("⚠️ La columna no se llama 'rango'. Columnas disponibles:", informacion[0].keys() if informacion else "vacío")
    ...
    def limpiar_campos(self):
        ...
        #tipo_ale = next((r["id_alergia"] for r in self.alergias if r["nombre_alergia"] == tipo_alergia),None)
        self.entry_tipo_aislamiento_paciente.set('Elige un Aislamiento')
        self.entry_texto_aislamientos_paciente.delete("1.0", "end")
        #tipo_aisla = next((r["id_aislamiento"] for r in self.aislamientos if r["nombre_aislamiento"] == tipo_aislamiento), None)
        self.entry_estado_paciente.set('Elige un Estado')
        #estado2 = self.entry_estado_paciente.set('Modificado')
        #id_estado2 = self.obtener_id_estado('Modificado')
        ...
        modalidad = self.entry_modalidad.set('Elige una Modalidad')
        ...
        causal_retraso = self.entry_list_caus_retraso.set('Elige una Causal de Retraso')
