...
class IngresoEstados():
    ...
    def __init__(self, parent_window=None):
        ...
        self.estado_var.trace_add("write", self.actualizar_a_title)
    ...
    def ingreso_datos(self):
        ...
        campos1 = [
            ...
            {'label': 'Estado', 'placeholder': 'Ingrese el Estado','ancho': 100, 'alto': 26, 'tipo':'entry'},
        ...
        campos2 = [
            ...
            {'label': 'Ingresar', 'ancho': 100, 'alto': 30, 'color':'Lightblue', 'command': self.insertar_estado},
            {'label': 'Eliminar', 'ancho': 100, 'alto': 30, 'color':'Lightblue', 'command': self.eliminar_estado},
            {'label': 'Modificar', 'ancho': 100, 'alto': 30, 'color':'Lightblue', 'command': self.modificar_estado},
        ...
        for i, campo1 in enumerate(campos1):
            ...
            self.entry_estado = self.crear_entry(self.frame1, 
        ...
        self.sql_statement = """insert into estados (nombre_estado) values (%s)"""
    ...
    def actualizar_a_title(self, *args):
        ...
        texto_actual = self.estado_var.get()
    ...
    def insertar_estado(self):
        ...
        consulta_existencia = "SELECT COUNT(*) FROM estados WHERE nombre_estado = %s"
        
        self.db.cursor.execute(consulta_existencia, (estado,))
    ...
    def buscar_estado(self, *args):
        ...
        sql_buscar_estado = "SELECT nombre_estado FROM estados WHERE nombre_estado LIKE %s"
        self.db.cursor.execute(sql_buscar_estado, (f"%{estado}%",))
        ...
        self.textbox_resultados.bind("<ButtonRelease-1>", self.seleccionar_estado)
    
    def modificar_estado(self):
        
        # Obtener el valor del Entry
        estado_nuevo = self.estado_var.get().strip()
    ...
    def obtener_resultados_busqueda(self, estado):
        
        # Realiza la búsqueda en la base de datos y devuelve los resultados
        sql_buscar_estado = "SELECT id_estado, nombre_estado FROM estados WHERE nombre_estado LIKE %s"
        self.db.cursor.execute(sql_buscar_estado, (f"%{estado}%",))
        resultados = self.db.cursor.fetchall()
        ...
        estado_dict = {}
        for resultado in resultados:
            estado_dict[resultado[0]] = resultado[1]  # {id_alergia: nombre_alergia}
        
        return estado_dict
    
    def seleccionar_estado(self, event):
        ...
        if seleccion and seleccion != "No hay coincidencias":
            self.estado_var.set(seleccion)
            
            # almacenar el ID de la alergia seleccionada
            estado_id = self.obtener_id_estado_seleccionado(seleccion)
            if estado_id:
                self.estado_id_seleccionado = estado_id  # Almacenar el ID para futuras modificaciones
    
    def obtener_id_estado_seleccionado(self, estado_nombre):
        
        sql_buscar_id = "SELECT id_estado FROM estados WHERE nombre_estado = %s"
        self.db.cursor.execute(sql_buscar_id, (estado_nombre,))
        resultado = self.db.cursor.fetchone()
