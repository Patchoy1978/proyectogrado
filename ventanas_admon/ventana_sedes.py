...
class IngresoSedes():
    ...
    def __init__(self, parent_window=None):
        ...
        self.sede_var.trace_add("write", self.actualizar_a_title)
    ...
    def ingreso_datos(self):
        ...
        campos1 = [
            ...
            {'label': 'Sede', 'placeholder': 'Ingrese la Sede','ancho': 100, 'alto': 26, 'tipo':'entry'},
        ...
        campos2 = [
            ...
            {'label': 'Ingresar', 'ancho': 100, 'alto': 30, 'color':'Lightblue', 'command': self.insertar_sede},
            {'label': 'Eliminar', 'ancho': 100, 'alto': 30, 'color':'Lightblue', 'command': self.eliminar_sede},
            {'label': 'Modificar', 'ancho': 100, 'alto': 30, 'color':'Lightblue', 'command': self.modificar_sede},
        ...
        for i, campo1 in enumerate(campos1):
            ...
            self.entry_sede = self.crear_entry(self.frame1, 
        ...
        self.sql_statement = """insert into sedes (nombre_sede) values (%s)"""
    ...
    def actualizar_a_title(self, *args):
        ...
        texto_actual = self.sede_var.get()
    ...
    def insertar_sede(self):
        ...
        consulta_existencia = "SELECT COUNT(*) FROM sedes WHERE nombre_sede = %s"
        
        self.db.cursor.execute(consulta_existencia, (sede,))
    ...
    def buscar_sede(self, *args):
        ...
        sql_buscar_sede = "SELECT nombre_sede FROM sedes WHERE nombre_sede LIKE %s"
        self.db.cursor.execute(sql_buscar_sede, (f"%{sede}%",))
    ...
    def modificar_sede(self):
        
        # Obtener el valor del Entry
        sede_nueva = self.sede_var.get().strip()
    ...
    def obtener_resultados_busqueda(self, sede):
        # Realiza la búsqueda en la base de datos y devuelve los resultados
        sql_buscar_sede = "SELECT id_sede, nombre_sede FROM sedes WHERE nombre_sede LIKE %s"
        self.db.cursor.execute(sql_buscar_sede, (f"%{sede}%",))
        resultados = self.db.cursor.fetchall()
        ...
        sede_dict = {}
        for resultado in resultados:
            sede_dict[resultado[0]] = resultado[1]  # {id_alergia: nombre_alergia}
        
        return sede_dict
    
    def seleccionar_sede(self, event):
        ...
        if seleccion and seleccion != "No hay coincidencias":
            self.sede_var.set(seleccion)
            
            # almacenar el ID de la alergia seleccionada
            sede_id = self.obtener_id_sede_seleccionado(seleccion)
            if sede_id:
                self.sede_id_seleccionada = sede_id  # Almacenar el ID para futuras modificaciones
    
    def obtener_id_sede_seleccionado(self, sede_nombre):
        sql_buscar_id = "SELECT id_sede FROM sedes WHERE nombre_sede = %s"
        self.db.cursor.execute(sql_buscar_id, (sede_nombre,))
        resultado = self.db.cursor.fetchone()
